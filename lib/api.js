...
export function findHomePromotedArticlesAndRecipes(limit) {
  ...
  return Promise.all([promotedRecipes, promotedArticles])
    .then(
    promisesValues => {
...
export function getWebForm(uuid) {
  return jsonApi.get(`webform/${uuid}`)
}

export function submitWebForm(url, data) {
  return jsonApi.post(url, { "data" :data });
}
